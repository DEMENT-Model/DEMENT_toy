...
class Output():
    ...
    def __init__(self,runtime,data_init):
        ...
        Microbes_grid['N'].name = 0                                                     # Rename the series name to 0
        Microbes_grid['P'].name = 0 
        self.MicrobesSeries     = Microbes_grid['C'] 
        self.MicrobesSeries2    = Microbes_grid['N']                                           # Taxon-specific total biomass summed over the grid
        self.MicrobesSeries3    = Microbes_grid['P']
        #print(Microbes_grid['N']) 
    ...
    def output(self,ecosystem,day):
        ...
        #print(self.Taxon_count)
        ## Taxon biomass
        Microbes_grid           = ecosystem.Microbes.groupby(level=0,sort=False).sum()
        Microbes_grid['C'].name = day + 1
        Microbes_grid['N'].name = day + 1
        Microbes_grid['P'].name = day + 1
        self.MicrobesSeries     = pd.concat([self.MicrobesSeries, Microbes_grid['C']], axis=1, sort=False)
        self.MicrobesSeries2    = pd.concat([self.MicrobesSeries2, Microbes_grid['N']], axis=1, sort=False)
        self.MicrobesSeries3    = pd.concat([self.MicrobesSeries3, Microbes_grid['P']], axis=1, sort=False)
        #print(MicrobesSeries2)
        ...
        CUE_Taxon_grid = ecosystem.CUE_taxon.groupby(level=0,sort=True).sum()
        #CUE_Taxon_grid = CUE_Taxon_grid/test.dropna()
        CUE_Taxon_grid.name = day + 1
        self.CUE_TaxonSeries = pd.concat([self.CUE_TaxonSeries,CUE_Taxon_grid],axis=1,sort=True)
        #print(self.Taxon_count)
        #print(self.CUE_TaxonSeries/self.Taxon_count)
        #print((CUE_Taxon_grid)/(test.dropna())[1])
        #print(test.dropna())
